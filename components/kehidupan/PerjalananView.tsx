...
/**
 * "Perjalanan" sub-view of the Kehidupan page — the life-trajectory
 * numbers (Personal Year forecast, Pinnacle/Challenge stages, Period
 * Cycles, Essence Cycle) plus the AI year-outlook hero. This is the old
 * /journey page body, moved verbatim into a component so it can live as
 * one tab of the merged Kehidupan page.
 */
export async function PerjalananView({
  profile,
  locale,
}: {
  profile: ProfileView;
  ...
  return (
    <div className="space-y-10">
                  ...
                  >
                    <header className="flex items-baseline justify-between gap-3">
                      <div className="flex items-baseline gap-2">
                        <span className="font-mono text-base font-semibold tabular-nums">{y.year}</span>
                    ...
                    {meaning ? (
                      <p className="mt-2 text-sm leading-relaxed text-neutral-700 dark:text-neutral-300">{meaning}</p>
                    ) : (
            ...
            {pinnacleRows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  {meaning ? (
                    <p className="mt-3 text-sm leading-relaxed text-neutral-800 dark:text-neutral-200">{meaning}</p>
                  ) : (
      ...
      <section className="space-y-4">
        ...
        {!profile.middleName ? (
          <p className="text-muted-foreground text-xs italic">{t('noMiddleNameNote')}</p>
        ) : null}
                    ...
                    >
                      <header className="flex items-baseline justify-between gap-3">
                        <div className="flex items-baseline gap-2">
                          ...
                          <span className="text-muted-foreground font-serif text-sm">{frame.letters}</span>
                        </div>
    ...
    <div className="space-y-1.5 py-3 first:pt-0 last:pb-0">
      <div className="flex items-baseline justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {secondary ? <CompoundReduced result={secondary} locale={locale} size="sm" /> : null}
